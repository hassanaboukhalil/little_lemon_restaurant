...
export default function Nav() {
  ...
  const [pageName, setPageName] = useState(() => {
    if (localStorage.getItem("page-name"))
  ...
  useEffect(() => {
    ...
    function changeNav(element) {
      if (element === navAbout.current || element === logo.current)
        element = navHome.current;
  ...
  return window.matchMedia("(min-width: 992px)").matches ? (
    <>
      ...
      <nav role="navigation" ref={navRef}>
        <ul>
          <li>
            <Link to="/" ref={navHome} aria-label="On Click">
              Home
            </Link>
          </li>
          <li>
            <Link to="/" ref={navAbout} aria-label="On Click">
              About
            </Link>
          </li>
          <li>
            <Link to="/menu-page" ref={navMenu} aria-label="On Click">
              Menu
            </Link>
          </li>
          <li>
          ...
          <li>
            <Link to="/contact-page" ref={navContact} aria-label="On Click">
              Contact
            </Link>
          </li>
          {isLoggedIn ? (
            <>
              <li>
                <Link to="/cart" ref={navCart} aria-label="On Click">
                  Cart
                </Link>
              </li>
              <li>
                <Link to="/" onClick={logout} aria-label="On Click">
                  Logout
                </Link>
              </li>
            </>
            ...
            <li>
              <Link to="/login" ref={navLogin} aria-label="On Click">
                Login
              </Link>
            </li>
          )}
    ...
    <>
      ...
      <div className="beside-nav" onClick={closeNav} ref={besideNavRef}></div>
    </>
  );
}
